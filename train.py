...
subset_size = min(7000, len(dataset))
...
for epoch in range(epochs):
    ...
    with torch.no_grad():
        for images, masks in val_bar:
            ...
            current_accuracy = 100 * correct_pixels / total_pixels

            val_bar.set_postfix({
                "pixel_acc": f"{current_accuracy:.2f}%"
            })
    ...
    print(
        ...
        f"loss: {avg_loss:.4f} "
        f"validation pixel accuracy: {pixel_accuracy:.2f}%"
    )
